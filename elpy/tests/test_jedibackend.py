...
from elpy.tests.support import (
    ...
    RPCGetCompletionLocationTests,
    RPCGetCompletionsTests,
    RPCGetDefinitionTests,
    RPCGetDocstringTests,
    RPCGetExtractFunctionDiffTests,
    RPCGetExtractVariableDiffTests,
    RPCGetInlineDiffTests,
    RPCGetNamesTests,
    RPCGetOnelineDocstringTests,
    RPCGetRenameDiffTests,
    RPCGetUsagesTests,
)
...
class TestRPCGetCompletions(RPCGetCompletionsTests, JediBackendTestCase):
    BUILTINS = ["object", "oct", "open", "ord", "OSError", "OverflowError"]


class TestRPCGetAssignment(RPCGetAssignmentTests, JediBackendTestCase):
    pass


class TestRPCGetCompletionDocstring(
    RPCGetCompletionDocstringTests, JediBackendTestCase
):
    pass


class TestRPCGetCompletionLocation(RPCGetCompletionLocationTests, JediBackendTestCase):
    pass


class TestRPCGetDocstring(RPCGetDocstringTests, JediBackendTestCase):
    def __init__(self, *args, **kwargs):
        super(TestRPCGetDocstring, self).__init__(*args, **kwargs)
        self.JSON_LOADS_REGEX = (
            r"loads\(s.*, cls.*, object_hook.*, parse_float.*, " r"parse_int.*, .*\)"
        )
...
class TestRPCGetDefinition(RPCGetDefinitionTests, JediBackendTestCase):
    ...
    def test_should_not_fail_if_module_path_is_none(self, Script):
        ...
        """
        locations = [mock.Mock(module_path=None)]
...
class TestRPCGetRenameDiff(RPCGetRenameDiffTests, JediBackendTestCase):
    pass


class TestRPCGetExtractFunctionDiff(
    RPCGetExtractFunctionDiffTests, JediBackendTestCase
):
    pass


class TestRPCGetExtractVariableDiff(
    RPCGetExtractVariableDiffTests, JediBackendTestCase
):
    pass


class TestRPCGetInlineDiff(RPCGetInlineDiffTests, JediBackendTestCase):
    pass
...
class TestRPCGetUsages(RPCGetUsagesTests, JediBackendTestCase):
...
class TestRPCGetNames(RPCGetNamesTests, JediBackendTestCase):
    pass


class TestPosToLinecol(unittest.TestCase):
    def test_should_handle_beginning_of_string(self):
        self.assertEqual(jedibackend.pos_to_linecol("foo", 0), (1, 0))
...
class TestLinecolToPos(unittest.TestCase):
    def test_should_handle_beginning_of_string(self):
        self.assertEqual(jedibackend.linecol_to_pos("foo", 1, 0), 0)
    ...
    def test_should_fail_for_line_past_text(self):
        self.assertRaises(ValueError, jedibackend.linecol_to_pos, "foo\n", 3, 1)
...
class TestRunWithDebug(unittest.TestCase):
    ...
    def test_should_re_raise(self, Script):
        ...
        with self.assertRaises(RuntimeError):
            jedibackend.run_with_debug(
                jedi, "test_method", 1, 2, arg=3, re_raise=(RuntimeError,)
            )
    ...
    def test_should_keep_debug_info(self, set_debug_function, Script):
        ...
        try:
            ...
            self.assertEqual(jedi_debug_info["script_args"], "1, 2, arg=3")
    ...
    def test_should_handle_source_special(self, set_debug_function, Script):
        ...
        try:
            ...
            self.assertEqual(e.data["jedi_debug_info"]["script_args"], "source=source")
    ...
    def test_should_set_debug_info(self, set_debug_function, Script):
        ...
        try:
            ...
            self.assertEqual(
                e.data["jedi_debug_info"]["debug_info"],
                ["[N] Notice", "[W] Warning", "[?] Other"],
            )
        else:
            self.fail("Fault not thrown")
    ...
    def test_should_not_fail_with_bad_data(self, Script, set_debug_function):
        ...
        def set_debug(function, speed=True):
            if function is not None:
                function(jedi.debug.NOTICE, "\xab")
