...
class TestGetPydocCompletions(unittest.TestCase):
    ...
    def test_should_find_objects_in_module(self):
        self.assertIn(
            ...
            elpy.pydocutils.get_pydoc_completions("elpy.tests.test_pydocutils"),
        )
    ...
    def test_should_return_none_for_inexisting_module(self):
        self.assertEqual([], elpy.pydocutils.get_pydoc_completions("does_not_exist"))

    def test_should_work_for_unicode_strings(self):
        self.assertIsNotNone(elpy.pydocutils.get_pydoc_completions("sys"))

    def test_should_find_partial_completions(self):
        self.assertIn(
            "multiprocessing", elpy.pydocutils.get_pydoc_completions("multiprocess")
        )
        self.assertIn(
            "multiprocessing.util",
            elpy.pydocutils.get_pydoc_completions("multiprocessing.ut"),
        )

    def test_should_ignore_trailing_dot(self):
        self.assertIn("elpy.pydocutils", elpy.pydocutils.get_pydoc_completions("elpy."))
...
class TestGetModules(unittest.TestCase):
    ...
    def test_should_catch_import_errors(self, safeimport):
        def raise_function(message):
            raise elpy.pydocutils.ErrorDuringImport(message, (None, None, None))
